...
func (store *Store) VerifyAndConsume(ctx context.Context, userID int64, code, secret string) (bool, error) {
	...
	numRowChanged, dbErr := result.RowsAffected()
	if dbErr != nil {
		return false, fmt.Errorf("count consumed TOPT time steps: %w", dbErr)
	}
	return numRowChanged == 1, nil
}
...
func (store *Store) ConsumeBackupCode(ctx context.Context, userID int64, code string) (bool, error) {
	...
	numRowChanged, dbErr := result.RowsAffected()
	if dbErr != nil {
		return false, fmt.Errorf("count consumed TOTP backup codes: %w", dbErr)
	}
	return numRowChanged == 1, nil
}
